                ...
                <div class="card mb-4">
                    <div class="card-body table-responsive">
                        <button class="btn btn btn-primary" data-bs-toggle="modal" data-bs-target="#insertModal"
                            style="margin-bottom: 10px;">新增分類</button>
                        <table class="table table-bordered">
                            ...
                            <tbody class="align-middle text-center">
                                <tr v-for="(couponcategories, couponcategoriesindex) in couponcategoriess" :key="couponcategoriesindex">
                                    <td><input type="checkbox" v-model="selectedCouponcategoriess" :value="couponcategories.couponcategoriesid">
                                    </td>
                                    <td>{{ couponcategories.couponcategoriesname }}</td>

                                    <td><button class="btn btn-outline-info" data-bs-toggle="modal"
                                            @click="openUpdateModal(couponcategories)" data-bs-target="#updateModal">修改</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        分類名稱<input v-model="updateSelectedCouponcategories.couponcategoriesname" type="text" class="form-control" required="required">
                        <span v-if="!updateSelectedCouponcategories.couponcategoriesname" class="text-danger">必填</span>
                    </div>                              
                    
                </div>
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary" @click="updateCouponcategories">送出</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="insertModal" tabindex="-1" aria-labelledby="insertModal" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        名稱<input v-model="couponcategories.couponcategoriesname" type="text" class="form-control" required="required">
                        <span v-if="!couponcategories.couponcategoriesname" class="text-danger">必填</span>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary" @click="insertCouponcategories">送出</button>
                </div>
            </div>
        </div>
    </div> 
...
const getcouponcategoriess = async () => {
    try {
        const response = await axios.get('http://localhost:8080/fithub/coupon-categories'); // 替換為實際的 API URL
...
const insertCouponcategories = async () => {
    try{
        const response = await axios.post('http://localhost:8080/fithub/coupon-categories', couponcategories, {
            headers: {
...
const updateCouponcategories = async () => {
    ...
    //     }
        ...
        const response = await axios.put('http://localhost:8080/fithub/coupon-categories/update', updateSelectedCouponcategories, {
            headers: {
                'Content-Type': 'application/json'
            }
        });
...
const deleteSelected = async () => {
    ...
    if (checkDelete) {
        try {
            ...
            const response = await axios.delete('http://localhost:8080/fithub/coupon-categories/delete/multiple', {
